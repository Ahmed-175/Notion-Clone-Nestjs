  ...
  return (
    ...
    >
      <div className="flex justify-between items-center gap-4">
        ...
        <div
          className=" rounded-full bg-black text-white w-5 flex 
        justify-center items-center h-5"
        >
      ...
      <div>
        {node.type && (
          <div
            className="w-fit   ml-3 border-l border-gray-800 pl-4 
          mt-1"
          >
            {node.children.map((child, index) => (
              <div
                ...
                key={index}
              >
                {child.type == "folder" ? <IoFolderOutline /> : <LuNewspaper />}
                {child.title}
              </div>
            ))}
          </div>
